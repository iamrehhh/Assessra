...
export default function TensesView() {
    ...
    const renderOptionButton = (key, text, item, isViewMode = false) => {
                    ...
                    <div className="bg-black/5 dark:bg-white/5 p-5 rounded-xl mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="space-y-1">
                                ...
                                {Object.entries(listI).map(([k, v]) => <p key={k}><span className="font-bold text-primary">{k}.</span> {v}</p>)}
                            </div>
                            <div className="space-y-1">
                                <p className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2">List II</p>
                                {Object.entries(listII).map(([k, v]) => <p key={k}><span className="font-bold text-primary">{k}.</span> {v}</p>)}
                            </div>
                        </div>
                    </div>
