...
export default function LeaderboardView() {
    ...
    const [activeTab, setActiveTab] = useState<'igcse' | 'alevel'>('igcse');
    ...
    useEffect(() => {
        fetch('/api/leaderboard')
            ...
            .then(d => { 
                ...
                // Default tab based on current user's level if they are on the board
                if (currentUser) {
                    if (d.alevel?.some((p: any) => p._id === currentUser)) {
                        setActiveTab('alevel');
                    } else if (d.igcse?.some((p: any) => p._id === currentUser)) {
                        setActiveTab('igcse');
                    }
                }
        ...
        <div className="space-y-10 animate-fade-in max-w-5xl mx-auto pb-12">
            ...
            <div className="flex justify-center gap-4 mb-8">
                <button
                    ...
                    className={`px-8 py-3 rounded-full font-bold text-lg transition-all ${activeTab === 'igcse' ? 'bg-primary text-background-dark shadow-[0_0_20px_rgba(132,204,22,0.4)]' : 'bg-black/5 dark:bg-white/5 text-text-muted hover:bg-black/10 dark:hover:bg-white/10'}`}
                >
                    IGCSE
                </button>
                <button
                    onClick={() => setActiveTab('alevel')}
                    className={`px-8 py-3 rounded-full font-bold text-lg transition-all ${activeTab === 'alevel' ? 'bg-primary text-background-dark shadow-[0_0_20px_rgba(132,204,22,0.4)]' : 'bg-black/5 dark:bg-white/5 text-text-muted hover:bg-black/10 dark:hover:bg-white/10'}`}
                >
                    A Level
                </button>
            </div>
            ...
            {!board?.length ? (
                <div style={{ textAlign: 'center', padding: '60px', color: '#aaa' }}>
                    <p style={{ fontSize: '1.3rem' }}>📭 No scores yet in {activeTab === 'igcse' ? 'IGCSE' : 'A Level'} — be the first on the board!</p>
                </div>
            ) : (
                <>
                    {/* Top 3 podium cards */}
                    {board.length >= 3 && (
                        <div className="flex justify-center items-end gap-4 pt-8 pb-4">
                            {[board[1], board[0], board[2]].map((player: any, i: number) => {
